...
TEST_CASE("Protein Processing & Analysis","[BA3]")
{
    ...
    SECTION("BA4f: Scoring cyclopeptide spectrum.")
    {
        auto input = getFileLines( dataFilePath("ba4f"));
        std::vector< int > spectrum;
        for( const std::string &strMass : rosalind::io::split( input.at( 1 ) , " "))
            spectrum.push_back( std::stoi( strMass ));

        auto actual = rosalind::ba4::scoreCyclicPeptideSpectrum( input.front() ,
                                                                 spectrum.cbegin() ,
                                                                 spectrum.cend());
        auto expected = std::stoi( getFileLines( outputFilePath("ba4f")).front());
        REQUIRE(  actual == expected );
    }
}
